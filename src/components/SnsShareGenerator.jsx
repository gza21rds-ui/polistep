...
export default function SnsShareGenerator({ visible, onClose, stats, statsToday = {}, user = {} }) {
  ...
  const drawCanvas = () => {
    ...
    ctx.fillText('地道な活動が、地域を変える。', canvas.width / 2, 1650);
    
    ctx.fillStyle = '#94A3B8';
    ctx.font = 'bold 35px sans-serif';
    ctx.fillText('一緒に活動してくれる仲間を募集しています！', canvas.width / 2, 1710);

    // Draw PoliStep Logo at bottom center
    const logoImg = new Image();
    logoImg.onload = () => {
      const logoSize = 70;
      ctx.drawImage(logoImg, canvas.width / 2 - logoSize / 2, 1760, logoSize, logoSize);
      
      ctx.font = 'bold 24px sans-serif';
      ctx.fillStyle = '#64748B';
      ctx.fillText('Generated by PoliStep', canvas.width / 2, 1860);
  ...
  const handleDownload = async () => {
    ...
    // Web Share API を使用して保存・共有ダイアログを出す（スマホ対応）
    if (navigator.share && navigator.canShare) {
      try {
        const response = await fetch(imgDataUrl);
        const blob = await response.blob();
        const file = new File([blob], `polistep_activity_${new Date().toISOString().split('T')[0]}.png`, { type: 'image/png' });
    ...
    // Share APIが使えない/失敗した場合は通常のダウンロード
      ...
      <div className="modal-content" style={{ maxWidth: '600px', width: '95%', padding: '2rem', textAlign: 'left', maxHeight: '95vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', marginTop: '2rem' }}>
          ...
          <div style={{ background: '#F8FAFC', padding: '1.5rem', borderRadius: '16px', border: '1px solid #E2E8F0', textAlign: 'center' }}>
            ...
            <div style={{ border: '1px solid #CBD5E1', borderRadius: '12px', overflow: 'hidden', margin: '0 auto 0.5rem', maxWidth: '240px', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)', background: '#020617' }}>
              <canvas ref={canvasRef} style={{ display: 'none' }}></canvas>
              {imgDataUrl ? (
                <img src={imgDataUrl} alt="SNS Report" style={{ width: '100%', height: 'auto', display: 'block', pointerEvents: 'auto' }} />
              ) : (
                <div style={{ width: '100%', aspectRatio: '9/16', background: '#020617' }}></div>
              )}
            </div>
            <p style={{ color: '#EF4444', fontSize: '0.8rem', fontWeight: 'bold', marginBottom: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.25rem' }}>
              👆 スマホの場合は画像を長押しして保存
            </p>
